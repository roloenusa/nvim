...
vim.cmd([[
  augroup jd_final_newline
    autocmd!
    autocmd BufWritePre * lua ensure_final_newline()
  augroup END
]])
...
-- Single autocmd with debouncing for resize events
...
vim.api.nvim_create_autocmd({ "BufEnter", "BufReadPost", "BufWritePost", "TextChanged", "InsertLeave" }, {
  callback = function()
    ...
    local start_char = "└"
    local end_char = "┘"
    local content_length = #start_char + #text + #end_char
